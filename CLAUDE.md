...
- `php` — Base PHP 8.4 CLI image with extensions (sockets, zip, pdo_pgsql) + Composer. Used for running one-off commands like `composer install`.
- `app` — RoadRunner image (extends php base + RoadRunner binary). The running application on port 8080.
- `postgres` — PostgreSQL 16 on port 5432 (user/pass/db: `app`/`app`/`app`)

**Key config:** `.rr.yaml` controls RoadRunner behavior (worker command, HTTP address, static file serving, worker pool size).
